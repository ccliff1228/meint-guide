...
비밀번호 대신 SSH 공개키(ed25519)를 사용하여 서버에 로그인하는 방법을 단계별로 안내한다. 비밀번호 접속 확인부터 키 생성, 서버 등록, 접속 편의 설정, 비밀번호 로그인 비활성화까지 전체 과정을 다룬다.

## 목차

- [서버 정보](#서버-정보)
- [1단계: 비밀번호 로그인 확인](#1단계-비밀번호-로그인-확인)
- [2단계: SSH 키 생성 (클라이언트)](#2단계-ssh-키-생성-클라이언트)
- [3단계: 공개키를 서버에 등록](#3단계-공개키를-서버에-등록)
- [4단계: 공개키 로그인 확인](#4단계-공개키-로그인-확인)
- [5단계: SSH 접속 편의 설정 (선택)](#5단계-ssh-접속-편의-설정-선택)
- [6단계: 비밀번호 로그인 비활성화 (서버 관리자)](#6단계-비밀번호-로그인-비활성화-서버-관리자)
