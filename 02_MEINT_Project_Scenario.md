...
MEINT(MES-ERP Integration) 프로젝트는 **제조업의 DX(디지털 전환)에서 AX(AI 전환)로의 패러다임 전환**을 실증하는 시나리오이다.

**문제(AS-IS):** 한 중소 제조기업이 Festo MES4와 더존 iCUBE ERP를 분리 운영 중이다. iCUBE는 소스코드 비공개·API 미제공·벤더 종속의 폐쇄적 시스템으로, MES4와의 프로그래밍 연동이 불가능하다. 작업자는 4개 이상의 시스템을 각각 수동 조작하며, 472개 Step의 RPA 스크립트로 우회하는 임시방편에 의존한다.

**해결(TO-BE):** 폐쇄적 iCUBE ERP를 **오픈소스 ERPNext로 전환**하고, **도메인 전문가 + 생성형 AI 도구(Claude Code + MCP 서버)** 조합으로 MES4-ERPNext 양방향 통합을 직접 구축한다. 전통적 DX에서 전문 개발팀이 6~12개월에 걸쳐 수행하던 통합 작업을, AX 패러다임에서는 도메인 전문가가 AI와 협업하여 수주 내에 완성한다.

**핵심 산출물:**

- **기반 설계** — 6계층 아키텍처, 공유 모듈(DB 커넥터, REST 클라이언트, 로거, 설정 관리)
- **마스터 동기화** — 7개 엔티티 매퍼, 양방향 동기화 엔진, JSON ID 매핑, 스냅샷 기반 삭제 감지
- **재고 동기화** — 폴링 서비스(30~60초), 잠금 관리자, 3단계 Parts-Item 매핑
- **통합 대시보드** — React 제조 대시보드(Work Order, Job Card, BOM, KPI)
- **AI 개발 인프라** — MCP 서버, 61개 슬래시 명령(9개 카테고리), 워크플로우 표준화
- **데모 데이터** — IoT 센서 모듈 제조 시나리오(16 품목, 4 BOM, 8 공정, 6 작업장)

**핵심 메시지:** MEINT에서 생성형 AI는 "보조 도구"가 아니라 **"핵심 개발 방법론"** 이다. 프로젝트의 거의 모든 산출물이 AI와의 협업으로 만들어졌고, 이 워크플로우 자체가 MCP 서버와 슬래시 명령으로 체계화되어 **재현 가능**하다.
